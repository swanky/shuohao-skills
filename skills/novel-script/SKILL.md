...
**有違規逐條修，改完重跑，直到透過。** 寫超了先砍動作節拍再壓臺詞；寫欠了補戲不注水——加衝突不加寒暄。需要一個美術沒登記的光照狀態時，去 art.json 裡補狀態再回來，不要繞過門。
...
`{baseDir}/examples/渡口-script.json`：《渡口》**全 6 集完整劇本**（9 場 123 句臺詞，每集冷開場兌現鉤子、都落在 120 秒 ±15% 內），對著 novel-outline 與 novel-art 的樣例全部品質門透過。當品質基準，也是自測夾具。
