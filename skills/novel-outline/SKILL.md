...
description: |
  ...
  14 道品質門全部由腳本確定性檢查（角色分檔上限、主場景上限隨集數動態、爽點間隔≤3集、每集鉤子懸念必填……），不靠模型自覺；
...
輸入一本小說 + 目標參數，輸出短劇改編大綱五件套。**四件模型寫、一件腳本算**（資產清單從分集資料自動彙總）。

`{baseDir}` = 本檔案所在目錄。腳本 `{baseDir}/scripts/novel-outline.mjs`，零依賴，`node` 直接跑。

**邊界（不做的事）**：不寫劇本臺詞、不做分鏡、不出影像/TTS 提示詞。梗概是敘述體，出現引號對白就是越界——`validate` 會攔。想從小說拆角色設定（畫像/形象提示詞/設定圖），那是 `novel-characters` 的活。
...
**人物表從原文拆**——大綱是角色設定的上游，`characters` 塊定下的分檔、人物線與來源，下游 `novel-characters` 直接拿去當角色清單，不用再判斷一遍誰重要。

例外是使用者手上已經有 `cast.json`（此前單獨跑過 `novel-characters`）：那就拿來當人物原料，角色、別名、關係都是現成的，不用重拆原文。分檔按 `importance` 反向對映：protagonist/major → `lead`，supporting → `support`，minor → `functional`。
...
按章節標題分卷（預設每卷 15 章，`--per-volume` 可調），識別不出章節就按字數切。列印 `{"volumes": N, ...}`；`truncated: true` 就明確告訴使用者尾部沒掃到，別悶著。
...
報告介面預設中文；使用者要英文介面就加 `--lang en`（或在 outline.json 頂層寫 `lang` 欄位，`--lang` 優先）。只翻譯介面文案，資料內容（爽點型別、梗概、品質門文案）原樣出。
...
彙報一句話說清：幾集、幾個角色幾個場景、爽點分佈、報告路徑；被截斷或有沒過的門要明說。
...
- 報告介面內建中英（`--lang`，預設中文、或跟 outline.json 的 `lang` 欄位）：介面文案與品質門標籤翻譯，資料內容（爽點型別、梗概、人名）與門的失敗詳情保持原文
